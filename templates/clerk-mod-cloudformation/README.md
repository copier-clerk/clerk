...
CloudFormation / SAM infrastructure overlay for clerk (spec 011, `iac.md` contract).
...
- **MANAGED** `.cfnlintrc.yaml` — cfn-lint config; `ignore_checks` comes from `cfnlint_ignore_rules`. Byte-identical on `clerk reproduce`.
...
```
clerk init --module clerk-mod-cloudformation
```

On first run, the `aws cloudformation validate-template` task runs only when `aws_validate=true`. The parameter files are seeded with `test -f` guards so they are never overwritten by a later `clerk reproduce`.
