...
steering/ADR scaffolding are each deferred to their own future `clerk-mod-*`
modules.
...
Each entry is an APM dependency locator — the same inline-source shape the clerk
...
`clerk-mod-apm` presupposes **≥ 1 package** (FR-002b). Phase 1 (the skill) must
...
Prefer clerk (handles trust, ordering, answer threading, `today` injection, and
...
copier copy --trust \
  ...
  https://github.com/copier-clerk/clerk-mod-apm.git <destination>
```
...
`clerk-mod-apm` declares its dependency edges as empty `when:false` hidden
