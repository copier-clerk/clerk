...
| `.pre-commit.d/bailiff-mod-api.yaml` | **managed** | spectral-lint hook fragment; merged by `bailiff-mod-precommit`; inert when `hook_manager=none` |
...
| `hook_manager` | str | `_external_data.precommit` | `none` = hook fragment inert |

Facts are read via `_external_data` aliases (spec 014). `bailiff-mod-base` and
`bailiff-mod-precommit` are hard data-dependencies: both must be present in the
selection.

Zero `_tasks`; reproduce needs no toolchain or network.

Edge: `depends_on: [bailiff-mod-base, bailiff-mod-precommit]`, `phase: normal`.
