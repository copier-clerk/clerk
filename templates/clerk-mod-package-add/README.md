...
- `run_after: [clerk-mod-base]` — base fully applied before this overlay.
- Gated on `layout == 'monorepo'`; all tasks are no-ops on `single` layout.
- `js_pkg_manager` is threaded from clerk-mod-base via `default: "{{ js_pkg_manager }}"`.
...
```sh
uv run scripts/clerk.py init --run-spec <run-spec with [clerk-mod-base, clerk-mod-package-add]>
```

Or copier-only (monorepo projects only):

```sh
copier copy --trust https://github.com/copier-clerk/clerk-mod-package-add.git <destination>
```
