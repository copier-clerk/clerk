...
- `run_after: [clerk-mod-base]` — base applies first; `project_name` is
...
```sh
uv run scripts/clerk.py init --run-spec <run-spec with [clerk-mod-base, clerk-mod-go]>
```

Copier-only (standalone; renders with defaults):

```sh
copier copy --trust https://github.com/copier-clerk/clerk-mod-go.git <destination>
```
