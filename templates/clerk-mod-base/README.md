...
The thinned universal scaffold + identity/license for every clerk project
...
| `.github/` | **managed** | Issue/PR templates, CODEOWNERS, dependabot — when `github_host=true`. No `workflows/` (that's clerk-mod-ci). |
...
| `.gitignore` | **task-output** (gitnr 0.3.0) | Generated from `gitignore_stack`. Init-only-guarded via `.clerk-base-init-done`. |
...
| Subtree | Moved to |
|---|---|
| `.agents/` + `.codex/` | clerk-mod-agentic |
| `infrastructure/` | IaC modules |
| `.github/workflows/` | clerk-mod-ci |
| `specs/` | clerk-mod-speckit |
