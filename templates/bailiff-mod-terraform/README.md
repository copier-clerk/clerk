...
works standalone or `run_after` [`bailiff-mod-base`](https://github.com/bailiff-io/bailiff-mod-base)
...
- `run_after: [bailiff-mod-base]` (when:false hidden answer) — base applies first
...
- `mise_tools` and `hook_blocks` are **agent-frozen union answers** injected by
  ...
  + `tflint` tokens; `bailiff-mod-base` is the single writer of `.mise.toml` and
  `bailiff-mod-precommit` is the single writer of the hook config file (M1).
