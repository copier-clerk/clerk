"""spec 007 SC-005, Q5 (T014): [stub_base, clerk-mod-apm] composes as a layer.

A [stub_base, clerk-mod-apm] selection:
...
- commits NO clerk-specific recipe file to the project (spec-010 invariant).

007 declares no hardcoded base (Q5): clerk-mod-apm's edges are empty, so ordering
here is driven by the STUB BASE declaring run_before: [clerk-mod-apm] (see the
...
from clerk import ordering, runner, trust
...
def _selection(
    apm_stub_base: TemplateRepo, clerk_mod_apm: TemplateRepo
...
def test_stub_base_orders_before_apm_and_threads(
    ...
    dest = tmp_path / "proj"
    runner.init_many(_selection(apm_stub_base, clerk_mod_apm), str(dest), today="2026-07-13")
    ...
    # Each layer committed its own answers file; NO clerk-specific recipe file exists.
    assert (dest / ".copier-answers.clerk-mod-stub-base.yml").is_file()
    assert (dest / ".copier-answers.clerk-mod-apm.yml").is_file()
    # spec-010 invariant: no clerk-authored order/recipe file in the project.
    recipe_like = [
        p.name
        for p in dest.iterdir()
        if p.name.startswith(".clerk") and p.name.endswith((".yml", ".yaml", ".json"))
    ]
    assert recipe_like == [], f"unexpected clerk recipe file committed: {recipe_like}"
...
def test_reproduce_recomputes_same_order(
    ...
    for af in ("clerk-mod-stub-base", "clerk-mod-apm"):
    ...
    assert order == ["clerk-mod-stub-base", "clerk-mod-apm"], f"recomputed order wrong: {order}"
