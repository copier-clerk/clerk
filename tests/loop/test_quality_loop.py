...
The reproduce test pairs with bailiff-mod-base (run_after edge) because reproduce_many
...
def _init_with_base(
    ...
    selection: list[tuple[TemplateRecord, dict[str, Any]]] = [
        (
            _record("demo/bailiff-mod-base", base, ["project_name"]),
            {
                "project_name": "myapp",
                "org": "acme",
                "license": "mit",
        ...
        (
            _record("demo/bailiff-mod-quality", quality, ["quality_languages"]),
...
def test_quality_managed_byte_identical_on_reproduce(
    ...
    bailiff_mod_quality: TemplateRepo,
    tmp_path: Path,
) -> None:
    """SC-002: MANAGED file byte-identical on reproduce (Constitution III).

    Pairs with bailiff-mod-base because reproduce_many recomputes the DAG from
    committed answers + edges, and the run_after: [bailiff-mod-base] edge would
    be dangling if base were absent from the project.
    """
    dest = tmp_path / "proj"
    _init_with_base(bailiff_mod_base, bailiff_mod_quality, dest, ["rust", "python"])

    quality_file = dest / _QUALITY_FILE
    assert quality_file.is_file(), ".agents/hooks/quality-languages not present after init"
    before = _digest(quality_file)
    answers_before = _digest(dest / ".copier-answers.bailiff-mod-quality.yml")
    ...
    assert _digest(dest / ".copier-answers.bailiff-mod-quality.yml") == answers_before, (
