...
- **`clerk-mod-apm`** — wires an [APM](https://microsoft.github.io/apm) package
  layer into the project. It renders a **managed** `apm.yml` (byte-reproducible)
...
**The `clerk-mod-apm` step (spec 007 / FR-010).**
...
- **You build the `apm_packages` list (Q2).** `apm_packages` is a
  **runtime-injected list-typed answer** (`type: yaml`, no frozen `choices:`).
  ...
  override. You may seed suggestions from clerk's own known set (speckit,
  ...
  inline-source shape clerk's own `apm.yml` uses; APM has no separate consumer
...
- **Post-init note (Q3).** `apm.yml` is byte-reproducible. `apm.lock.yaml` is a
  **task side-effect / external state** — regenerated by the pinned install task
  at reproduce and MAY differ across runs; only `apm.yml` is byte-reproducible.
