...
etc.) on every run. clerk needs a way to store GLOBAL, PER-MODULE default answers
...
2. **`_external_data` reading a user-global YAML** — requires `unsafe=True` for
   ...
   couples the template to a clerk-specific path (kills portability).
3. **clerk injects via the API** — clerk reads its own config and passes values
...
- **clerk reads its own config (`~/.config/clerk/defaults.yml` or equivalent),
...
- **Use `user_defaults=`, NOT `data=`, for user-changeable defaults** — soft
  ...
  (correct only for values clerk truly fixes, e.g. computed/derived answers).
- **Optionally merge copier's native `settings.yml defaults:`** as a fallback:
  clerk may `copier.load_settings()` and fold well-known fields (`user_name`,
...
- **No `unsafe=True`, no template changes, per-module scoping** — all under
  clerk's control; templates stay portable with zero clerk coupling.
- Beats `settings.yml defaults:` (flat-global, no per-module scoping) and
  ...
  + couples the template to a clerk path).

## Consequences

- clerk owns a small user-config store (`~/.config/clerk/`) and a mapping of
