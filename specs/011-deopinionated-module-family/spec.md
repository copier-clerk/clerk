# Feature Specification: De-opinionated bailiff-mod-* module family + new modules (spec 011)
...
**process-deterministic task-output** rather than config-consistent renders. This softens
...
releases, and the `bailiff-mod-apm` → `bailiff-mod-agentic` mirror tombstone, are irreversible
...
A developer selects a language overlay (e.g. `bailiff-mod-python` or `bailiff-mod-ts`) and is
...
**Independent Test**: Init `bailiff-mod-python` with `python_pkg_manager=pdm` and `python_layout=src`
...
1. **Given** `bailiff-mod-python` with `python_pkg_manager=uv`, `ruff_line_length=88`, **When** init,
...
2. **Given** `bailiff-mod-ts` with `js_pkg_manager=pnpm`, `ts_linter=biome`, **When** init, **Then**
...
A developer reproduces a project on a fresh machine. Files bailiff renders (config it owns) come back
config-consistent; language manifests that were generated by a native tool command come back
**process-deterministically** (equivalent, tool-version-pinned, not asserted config-equivalent);
seed-once/living files that the project has edited are not clobbered.

**Why this priority**: Reproduce is bailiff's headline guarantee. The native-commands decision changes
...
config re-renders config-consistently; `pyproject.toml` is regenerated by the pinned tool; an edited
`AGENTS.md` on a re-run is preserved.

**Acceptance Scenarios**:

1. **Given** a generated project, **When** reproduce onto a fresh checkout, **Then** bailiff-managed
   config files are config-consistent and tool-generated manifests are regenerated by the version-pinned
   tool (process-deterministic, not asserted config-consistent) — consistent with the amended Constitution III.
...
A developer opts into `bailiff-mod-agentic` and selects which coding agents to configure (Claude, Codex,
...
distinctive agentic wiring as one coherent module. It also folds in the released `bailiff-mod-apm`.

**Independent Test**: Init `bailiff-mod-agentic` with `agentic_targets=[claude, kiro]`, `mcp_config=true`
...
**Independent Test**: Init `bailiff-mod-ci` with `ci_host=github`, `ci_model=standard`, two active languages →
...
A developer adds IaC via the module matching their paradigm — `bailiff-mod-terraform` (Terraform or OpenTofu),
`bailiff-mod-cdk` (AWS CDK in their language), or `bailiff-mod-cloudformation` (raw CFN or SAM) — as a
...
**Independent Test**: Init `bailiff-mod-terraform` with `tf_flavor=opentofu`, `placement_dir=infrastructure` →
...
1. **Given** `bailiff-mod-terraform` `tf_flavor=terraform`, **When** init, **Then** the HCL skeleton + `.tflint.hcl`
   (managed) + seed-once `main.tf`/`backend.tf` are written and `terraform init` runs trust-gated.
2. **Given** `bailiff-mod-cdk` `cdk_language=python`, **When** init, **Then** `cdk init app --language=python` runs
   as a trust-gated task into `placement_dir`, never `cdk bootstrap`/`deploy`.
3. **Given** `bailiff-mod-cloudformation` `mode=sam`, **When** init, **Then** the seed template carries the SAM
   Transform + Globals; raw mode omits them.

### User Story 6 — Get a thin, unopinionated base scaffold (Priority: P1)

A developer selecting `bailiff-mod-base` gets a minimal universal skeleton (`docs/`, `scripts/`, `tests/`,
...
**Independent Test**: Init `bailiff-mod-base` alone → contains `docs/`+`scripts/`+`tests/`, no `infrastructure/`,
...
1. **Given** `bailiff-mod-base` with defaults, **When** init, **Then** the always-on dirs are created and the
...
- **Reproduce of a tool-generated manifest across tool versions**: pinned via `mise`; process-deterministic, not
  config-consistent — the amended Constitution III governs (FR-019). If the tool is absent, preflight fails loudly.
- **A module selected with an empty/none feature set** (e.g. `bailiff-mod-agentic` with no targets): renders cleanly,
...
- **`bailiff-mod-apm` consumers after the rename**: the old mirror is tombstoned with a deprecation pointer to
  `bailiff-mod-agentic`; this is a confirmed public action, not automatic.
...
- **FR-008** *(lifecycle)*: Each module MUST classify each output as **managed** (bailiff owns; re-rendered config-consistently),
  **seed-once/living** (`_skip_if_exists`; scaffolded once then project-owned), or **task-output** (process-deterministic,
  produced by a native/network task). Tool-owned manifests are task-output; bailiff-owned config (e.g. `.tflint.hcl`,
...
- **FR-011**: No new `src/bailiff/` code or `scripts/bailiff.py` verb is introduced (Constitution I / C-11); all behavior is
  copier questions, rendered files, `when:`/`when:false` edges, and trust-gated tasks.

### Functional Requirements — module set

- **FR-012** *(revise built modules)*: `bailiff-mod-base`, `bailiff-mod-python` MUST be revised to satisfy FR-001…FR-010.
  `bailiff-mod-apm` MUST be folded into `bailiff-mod-agentic` (FR-016) rather than revised in place. **Migration (critique M2):**
...
- **FR-012a** *(reproduce is not coupled to the toolchain — critique M3)*: Every preflight (`mise install`) and native-init
  `_task` MUST be **init-only-guarded** (a committed sentinel or a `test -f <manifest>` guard, as `bailiff-mod-base`'s LICENSE
  ...
  network present.
- **FR-013** *(build ported modules)*: The following MUST be authored: `bailiff-mod-ts`, `bailiff-mod-go`, `bailiff-mod-rust`,
  `bailiff-mod-precommit`, `bailiff-mod-quality`, `bailiff-mod-justfile`, `bailiff-mod-readme`, `bailiff-mod-stack-adr`,
  `bailiff-mod-github-repo`, `bailiff-mod-package-add`. Each de-opinionated per its grilling verdict.
- **FR-014** *(drop / defer — amended per 2026-07-14 critique)*: `worktreeinclude-write` (`bailiff-mod-worktree`) and
  `env-example` (`bailiff-mod-env`) MUST NOT be built (dropped: niche/too-opinionated). **`bailiff-mod-org-policy` is DROPPED
  from the 011 build set** (critique R1): it is inert until an org-source-fetch module exists (which is not planned here),
  so it ships in a future org-governance spec alongside its only consumer, not as a dead module now. `bailiff-mod-speckit`
  remains a SEPARATE module (owns `specs/`), not part of this build set.
- **FR-015** *(thin base)*: `bailiff-mod-base` always-on outputs MUST be `docs/` (with a lean `docs_subdirs` toggle default
...
- **FR-016** *(bailiff-mod-agentic)*: A NEW `bailiff-mod-agentic` module MUST roll up agentic config as a copier template:
  ...
  supersedes `bailiff-mod-apm` (reopens spec 007; migrates apm's FRs; renames the released mirror — see FR-020).
- **FR-017** *(CI — TWO host modules, amended per critique R3)*: CI is split into TWO separate modules —
  **`bailiff-mod-ci-github`** and **`bailiff-mod-ci-gitlab`** (they share almost no render; a single host-branched module was
  ...
  is NOT offered. `bailiff-mod-ci-gitlab` additionally has `gitlab_tier` `[free, premium_ultimate]` default free (merge-queue
...
- **FR-018** *(IaC family — three modules)*: Three NEW separate modules MUST be authored (different paradigms, no shared
  template content): `bailiff-mod-terraform` (`tf_flavor [terraform, opentofu]` default `terraform`; HCL skeleton; managed
  versions.tf/.tflint.hcl; seed-once main/backend; `.terraform.lock.hcl` task-output via trust-gated init; tflint+trivy,
  no tfsec; env-per-dir not workspaces; no Terragrunt); `bailiff-mod-cdk` (AWS CDK; `cdk_language` `[typescript, python, go,
  ...
  `bailiff-mod-cloudformation` (`mode [raw, sam]` default raw; YAML-only render; seed-once template.yaml + per-env parameter
...
- **FR-019** *(reproduce-model reconciliation)*: The plan phase MUST amend Constitution III and write an ADR recording that
  tool-generated manifests (via native init under FR-007) are **process-deterministic task-output**, not config-consistent
  renders — extending the existing LICENSE/gitnr/apm-lock precedent — and the tradeoff (authentic new-project setup +
  mise-pinned determinism vs strict config-consistency). No module may be released under 011 until this reconciliation lands.
- **FR-020** *(apm rename / tombstone)*: Folding `bailiff-mod-apm` into `bailiff-mod-agentic` MUST create the new module + mirror
  and tombstone the released `bailiff-io/bailiff-mod-apm` mirror with a deprecation pointer. This is an irreversible public
...
- **MI-1** *(version auto-updater)*: A CI-driven job in the bailiff repo that checks upstream releases and bumps the version
  `choices:`/defaults across modules. Bailiff-repo tooling, scoped separately; not a `bailiff-mod-*` module. Flagged for its own
  small spec.
...
- **bailiff-mod-\* template**: one module — copier template under `templates/bailiff-mod-<name>/`, fanned out by 008b.
...
- **SC-001**: Every consequential tooling decision a competent team differs on is a choice with a sane default; a spot-check
  ...
  is offered.
- **SC-002** *(reworded per critique R5)*: A generated project reproduces with bailiff-managed files config-consistent and
  ...
  manifests (never a strict bailiff guarantee for task-output), not reproduce fidelity.
- **SC-003**: `bailiff-mod-agentic` produces working config for any subset of `[claude, codex, opencode, kiro]` and installs
  packages via native marketplace and/or APM; the empty selection renders with no error.
- **SC-004**: `bailiff-mod-ci` renders a valid, correctly-gated workflow for each of the 5 models on both GitHub and GitLab,
...
- **SC-006**: `bailiff-mod-base` generates only the thinned always-on set at defaults; moved-out and dropped directories are
...
- `mise` is available (or installable) in the environments where generated projects are scaffolded; requiring the tool at
  scaffold time is acceptable because bailiff is focused on new-project setup.
- Spec 007 is reopened by this spec (apm fold-in); its apm-specific FRs migrate to `bailiff-mod-agentic`.
