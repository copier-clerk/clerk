# Implementation Plan: De-opinionated clerk-mod-* module family + new modules (spec 011)
...
Deliver the de-opinionated clerk-mod-* family: revise 2 built modules, fold apm into a new
...
staying pure template content (no new `src/clerk/` code — C-11). Two governance deliverables
...
task-output (FR-019); **(b)** a spec-007 reconciliation (apm FRs migrate to `clerk-mod-agentic`).
...
`scripts/check_modules.py` (Python 3.11+) is the lint; **no new `src/clerk/` module, no new
`scripts/clerk.py` verb** (C-11 / FR-011).
...
|---|---|---|
| **I — Skills + Templates + Minimal Glue (C-11)** | PASS | Pure template content under `templates/clerk-mod-*/`. No new `src/clerk/` module or `scripts/clerk.py` verb. `mise`/native-commands/multi-model-CI/agentic-rollup are all copier questions + rendered files + trust-gated tasks — no copier gap requiring glue (FR-011). |
...
```text
templates/
├── clerk-mod-base/            # REVISED — thinned scaffold + de-opinionation questions
├── clerk-mod-python/          # REVISED — PM/version/ruff/layout choices; native uv init
├── clerk-mod-agentic/         # NEW — agentic config rollup (apm folded in)
├── clerk-mod-ts/              # NEW
├── clerk-mod-go/              # NEW
├── clerk-mod-rust/            # NEW
├── clerk-mod-precommit/       # NEW — owns the hook_manager threading contract
├── clerk-mod-quality/         # NEW
├── clerk-mod-justfile/        # NEW
├── clerk-mod-readme/          # NEW
...
# clerk-mod-apm/ is REMOVED from templates/ (folded into agentic); its released mirror is
...
specs/007-agentic-module/spec.md  # amended: apm FRs migrate to clerk-mod-agentic
...
**Structure Decision**: mirror the proven `clerk-mod-python` shape (`_subdirectory: template`,
answers-file `.jinja`, `when:false` edges, trust-gated `_tasks`). Each module authored in-monorepo;
008b fans each out to `copier-clerk/clerk-mod-<name>`. `clerk-mod-apm` is deleted from `templates/`
and its cog/catalog registration migrates to `clerk-mod-agentic`.
...
3. **Native-command scaffold pattern (FR-007 / ADR-0007)**: initial manifest via the tool's own
   ...
   deps later = native `add` command (package-add). Config clerk owns (tsconfig, `.tflint.hcl`,
   ruff-beyond-init, CI files) stays managed render.
4. **hook_manager threading contract (precommit owns it)**: `clerk-mod-precommit` owns
...
5. **Agent-frozen `--data` facts (FR-010)**: `clerk-mod-ci` and `clerk-mod-stack-adr` sort
...
- **FR-019 — Constitution III amendment + ADR-0007**: III already carves out "process-deterministic,
  ...
  (authentic new-project setup + mise pinning vs strict byte-identity) and the boundary (config clerk
...
- **Spec-007 reconciliation (FR-016/FR-020)**: amend 007's Q1/OQ-007-b/f/D-007-4 to record the hybrid
  ...
  lockfile/empty-set-refusal) migrate to `clerk-mod-agentic`'s feature set, with the empty-set refusal
