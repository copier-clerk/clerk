# Contract — clerk-mod-agentic (NEW; folds in clerk-mod-apm)

Agentic-config rollup. References [_cross-cutting.md](./_cross-cutting.md). Reopens spec 007
(apm FRs migrate here; empty-set refusal DROPPED). Supersedes `clerk-mod-apm` (mirror tombstoned
...
Amend 007 Q1/OQ-007-b/f/D-007-4: hybrid resolution — agentic rollup + apm folded + speckit separate. 007's apm FRs (apm.yml/install/lockfile) migrate here; empty-set refusal (007 FR-002b) DROPPED. Old `copier-clerk/clerk-mod-apm` mirror tombstoned (confirmed public action, not authoring).
