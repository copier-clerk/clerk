# Contract — clerk-mod-python (REVISE)

Python language overlay, de-opinionated + native-command scaffolded. References
[_cross-cutting.md](./_cross-cutting.md). `run_after: [clerk-mod-base]`.
...
| today | str | "" | injected |
| run_after | yaml when:false | [clerk-mod-base] | (+ clerk-mod-precommit when it lands, for hook order) |
...
- **ruff pre-commit hook** — contributed to clerk-mod-precommit via the hook_manager threading contract (single writer); not written here.
