...
- bailiff-mod-base — thinned (see Base below)
- bailiff-mod-python — add PM/version/ruff/layout/test choices
- bailiff-mod-apm — **FOLDS INTO bailiff-mod-agentic** (rename/tombstone released v0.1.0 mirror)

### New / to-build
- bailiff-mod-ts, bailiff-mod-go, bailiff-mod-rust (language overlays)
- bailiff-mod-precommit, bailiff-mod-quality (quality — quality stays SEPARATE per user)
- bailiff-mod-justfile (tooling)
- bailiff-mod-env, bailiff-mod-readme, bailiff-mod-stack-adr (agent-tier docs/tooling)
- bailiff-mod-ci (multi-model, GitHub Actions + GitLab CI) — owns .github/workflows OR .gitlab-ci.yml
- bailiff-mod-github-repo, bailiff-mod-package-add (integration/monorepo)
- bailiff-mod-org-policy (last; no-op until org-source-fetch exists)
- bailiff-mod-agentic (NEW rollup: Claude/Codex/OpenCode/Kiro-IDE/Kiro-CLI config + MCP + native marketplace + APM install; owns .agents/ + .codex/)
- bailiff-mod-iac (NEW: terraform/opentofu/pulumi/cdk/cfn/ansible; owns infrastructure/; layout-independent overlay) — GRILL+RESEARCH pending
- bailiff-mod-speckit (SEPARATE; owns specs/)

### Dropped
- worktreeinclude-write (bailiff-mod-worktree) — niche, deferred indefinitely
...
## bailiff-mod-base (thinned — ratified)
...
- MOVED OUT: .agents/ + .codex/ → bailiff-mod-agentic; infrastructure/ → bailiff-mod-iac; .github/workflows/ → bailiff-mod-ci; specs/ → bailiff-mod-speckit.
...
## bailiff-mod-agentic (NEW — design done, apm folded in)
...
## bailiff-mod-ci (multi-model — GitHub design done+grilling; GitLab researching)
...
- package-add: use NATIVE add commands (pnpm/bun/uv/cargo/go add) as trust-gated tasks — tool writes pnpm-workspace.yaml vs package.json workspaces[] itself. bailiff scaffolds new pkg dir + seed.
...
- CONSEQUENCE: language manifests (pyproject/package.json/Cargo.toml/go.mod) become TASK-OUTPUT (process-deterministic), NOT seed-once managed renders. Reproduce re-runs the init command (like the gh LICENSE fetch) — NOT config-consistent. Acceptable under Constitution III (task side-effects process-deterministic) but a DEPARTURE from Phase-0 bailiff-mod-python's rendered-seed-once pyproject. bailiff-mod-python must be REVISED to this model too.
- Config files bailiff owns (tsconfig, .golangci.yml, ruff config beyond what init writes, rustfmt/clippy) stay MANAGED renders where the tool doesn't own them; tool-owned manifests are task-output. Draw the line per file at build.
- Determinism note: pin tool VERSIONS via mise (.mise.toml) so init output is stable across runs; still process-deterministic not config-consistent. Flag in spec reconciliation (touches Constitution III framing + FR-005/FR-005a).
...
### bailiff-mod-terraform (RESEARCHED — ready to grill+build)
...
- Native command: `terraform init` / `tofu init` (network, trust-gated task via mise). Tooling: tflint + trivy config (tfsec DEAD→trivy), terraform_fmt via antonbabenko/pre-commit-terraform → contributes to bailiff-mod-precommit. NO workspaces (env-per-dir idiomatic), NO Terragrunt (future).
- Generic HCL skeleton (no cloud_provider question — user adds provider block); backend seed = commented local + S3 examples.

### bailiff-mod-cdk (AWS CDK — RESEARCHED; needs grill)
...
### bailiff-mod-cloudformation (RESEARCHED; needs grill)
...
- bailiff-model fit: STRONG (declarative YAML render = the whole product; seed-once like pyproject; no new bailiff code).

## bailiff-mod-ci (GRILLED — revisions to apply; GitLab design done, needs its own grill)
...
- Apply all GitHub-grill + GitLab-grill render fixes (see bailiff-mod-ci sections above) at build: gate suppressed on minimal; optional:true on needs-to-change-gated-jobs; pinned images; coupled interruptible; literal compare_to; canonical workflow:rules; fallback_keys; zero-lang guard emits when monorepo_tool!=none.

## Meta-features (separate scope, not modules)
- Version auto-update via CI: scheduled job bumping choices:/defaults lists (python/go/rust/node/action pins). Bailiff-repo tooling. Flag as its own small spec item.

## Spec reconciliation TODO
- Spec 009: FR-011 relaxed, FR-014/FR-015 added, roadmap 2.2.0 — DONE (commit 4c570b0).
- Spec 007: REOPEN — apm folds into bailiff-mod-agentic; amend Q1/OQ-007-b/f/D-007-4; apm FRs migrate.
...
- bailiff-mod-iac: full grill+research (tools, state backend, layout, placement).
