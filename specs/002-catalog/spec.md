# Feature Specification: clerk catalog — user-owned sources, runtime discovery + injection
...
`scripts/clerk.py`, and template selection is the phase-1 agent's job backed by a
...
clerk a skill-bundled copier wrapper (`scripts/clerk.py` + `SKILL.md`, no console
script). Spec 002 lets a user point clerk at **their own set of source repos** and
...
`scripts/clerk.py` gains `catalog` verbs to create, list, add to, remove from, and
**refresh** (discover + verify) that file. Discovery reuses the existing static
`copier.yml` parser (`src/clerk/discovery.py`) against each source, so the derived
...
naming a template that does not exist), `scripts/clerk.py` provides a
...
guardrail a selector template would provide, without a committed clerk artifact or
...
1. **Catalog = plain user-owned file, managed by `scripts/clerk.py`** — NOT a
...
3. **Selection is the agent's job; a deterministic gate validates it.** The skill
   presents the verified listing and collects the user's pick. `scripts/clerk.py`
...
4. **Full-id namespacing always** (`<catalog>/<template>`). clerk supports **one or
...
6. **No git submodules, no `catalog.yml` committed into generated projects, no
   ...
   runtime; nothing clerk-authored lands in a generated project (the 010 invariant
   holds).
...
### US1 — Point clerk at my own sources and see the templates (Priority: P1)
...
run `scripts/clerk.py catalog list`; assert the output enumerates each source's
...
**Independent Test**: on a machine with no catalog file, run `scripts/clerk.py
...
- **FR-001**: The catalog MUST be a **plain user-owned file** listing source repos
  ...
  same user-config resolution clerk already uses (mirroring `trust.py`'s
  `platformdirs`/env approach) and MUST be overridable with an explicit
  `--catalog PATH`.
- **FR-002**: `scripts/clerk.py` MUST provide catalog-management verbs to **create
  (if absent), list, add, and remove** sources. `add`/`remove` MUST be idempotent
  and MUST NOT destroy unrelated entries.
- **FR-003**: `scripts/clerk.py catalog refresh`/`list` MUST derive the available
...
- **FR-004**: Templates MUST be identified by **full-id `<catalog>/<template>`**.
  clerk MUST support one or more catalog pointers and MUST NOT provide an
...
- **FR-006**: `scripts/clerk.py` MUST provide a **deterministic selection-validation
...
- **SC-005**: No clerk-specific artifact is written into any generated project and
...
`--data`. Spec 010 reshaped clerk to skill-bundled with **no committed clerk
...
- **repos-collector template → plain user-owned catalog file** managed by
  `scripts/clerk.py` (FR-001/FR-002). No template whose only job is holding a list;
...
- **Q-002a — Catalog file format**: TOML (idiomatic config, matches `pyproject.toml`,
  ...
  for the clerk-owned config; resolve at planning. Either way it is plain,
...
- Constraints: C-01, C-04, C-06, C-11 (glue only where copier cannot). Delivery
  shape from spec 010 (skill-bundled `scripts/clerk.py`, no committed clerk artifact).
