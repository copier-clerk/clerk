# Implementation Plan: clerk catalog — user-owned sources, runtime discovery + injection
...
Give clerk a **user-owned catalog** of source repos and a **deterministic,
static** way to list the templates they offer, plus a **validation gate** on
selection — all inside the spec-010 skill-bundled shape (`scripts/clerk.py` +
`SKILL.md`, no console script, no committed clerk artifact). No repos-collector
...
- **Catalog file** — a plain **TOML** file of named catalog pointers, each listing
  ...
  `platformdirs`/env resolution `trust.py` uses (`user_config_path("clerk")`),
...
- **`scripts/clerk.py catalog` verbs** — `init` (create if absent), `list`, `add
...
**Language/Version**: Python 3.11+ (`scripts/clerk.py` + `src/clerk/`). TOML read is
...
comments on round-trip — acceptable because the catalog is clerk-managed config and
...
`user_config_path("clerk")/catalog.toml`, or `--catalog PATH`, or `COPIER`-style
...
tested via subprocess (`uv run scripts/clerk.py catalog …`) + unit tests on the new
...
no committed clerk artifact (010 invariant); no repos-collector/selector template
...
|---|---|---|
| I — Skills + templates + minimal glue | PASS | One new `src/clerk/catalog.py` module + `catalog` verbs on `scripts/clerk.py`; the catalog is user config, not a tool artifact. Glue justified by a copier gap (copier has no catalog/multi-source concept — C-11). |
...
`specs/001-clerk-vertical-slice/research.md`. A data-model doc is unnecessary — the
...
scripts/clerk.py         # EXTEND: add the `catalog` subcommand group
                         #   (init/list/add/remove/refresh/validate); dispatch to clerk.catalog.
                         #   Reuse the existing argparse + error→exit (0/1/2/3) structure.

pyproject.toml           # EDIT: `uv add tomli-w`. (tomllib is stdlib.)

skills/clerk/SKILL.md    # EXTEND: a catalog step before discovery/selection —
...
**Structure Decision**: One new `src/clerk/catalog.py` (the copier-gap glue: copier
has no multi-source/catalog concept), surfaced through new `catalog` verbs on the
existing `scripts/clerk.py`, reusing `discovery.discover` verbatim for the per-source
parse and `trust.py`'s user-config-path pattern. No new engine, no template, no
adapter. The catalog is user config; nothing clerk-authored enters a generated
project.
