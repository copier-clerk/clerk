# Feature Specification: clerk upgrade — explicit version upgrade + copier migrations
...
clerk's role is the glue copier cannot supply: announce the from→to versions per
...
1. **`run_update` is copier's upgrade path; clerk does NOT re-implement the merge.**
   copier's 3-way merge handles the diff between template versions against any local
   edits (ADR-0001). clerk's role is to announce versions, drive the call in
...
4. **`_migrations` are update-only, trust-gated.** copier runs `_migrations` only
   ...
   execute as code → governed by the same trust surface as `_tasks`. clerk MUST
...
5. **New `_migrations` format only.** The deprecated per-stage `{"version": …,
   ...
   and is REJECTED by clerk discovery at upgrade time (Constitution VI, C-06).
...
6. **conflict=`inline` by default; surface conflicts to the user.**
   When `run_update` writes inline conflict markers (git-merge-style), clerk
...
7. **Announced from→to.** Before each layer's upgrade, clerk emits the from-version
...
8. **Brownfield adoption (no `.copier-answers.yml`) is OUT of scope.** This spec
   only upgrades projects that were already init-ed with clerk (have the committed
...
A developer has a project generated from `clerk-mod-base v1.0.0`. A new
`v1.1.0` tag is available. They run `scripts/clerk.py update` targeting
the project; clerk announces `v1.0.0 → v1.1.0`, calls `run_update`, and reports
...
3. **Given** source is untrusted, **When** upgrade, **Then** clerk refuses with
...
3. **Given** migration in deprecated `{"version": …, "before": …, "after": …}` form,
   **When** upgrade, **Then** clerk refuses at discovery with a clear error naming
...
produces inline conflict markers. clerk reports the conflicted paths and exits
...
exit non-zero; inline conflict markers present in the file; clerk names the
...
project that had only B at v1.0.0 surfaces this new dependency. Clerk alerts the
...
Upgrade a v1.0.0 single-layer project. Assert clerk surfaces the new dependency
...
1. **Given** upgraded template v1.1.0 declares new `depends_on: [C]`, **When**
   upgrade a project that does not have C, **Then** clerk surfaces this as a
   warning or error (exact behavior pending Q-006b).

### Edge Cases

- **No `.copier-answers.yml`**: refused immediately (no brownfield path here).
- **Already at the target version**: clerk reports "already at target, nothing to
  do"; exit 0.
...
- **Migration command fails**: copier propagates the failure; clerk surfaces it
...
- **`run_update` with `pretend=True`**: dry-run upgrade to preview changes without
  writing. Clerk must surface what would change (relies on copier's pretend mode).
...
- **FR-001**: clerk MUST provide an `update` verb in `scripts/clerk.py` that drives
  `run_update` per layer in dependency order (spec 003's DAG, re-solved at target
  versions).
- **FR-002**: clerk MUST announce the from→to version for each layer before any
  `run_update` call.
- **FR-003**: clerk MUST re-solve the ordering DAG against the target template
...
- **FR-004**: clerk MUST trust-gate upgrade identically to init: a template with
  `_migrations` or `_tasks` from an untrusted source MUST raise `UntrustedSourceError`
  (exit 3) before calling `run_update`.
- **FR-005**: clerk MUST enforce the new `_migrations` format: if static discovery
...
- **FR-006**: Migrate version-crossing check: clerk MUST only accept a `_migrations`
  entry where the version condition is `target >= entry_version > from_version`
  (matches copier's own filter; the check is copier's, clerk's discovery only
  validates the format).
- **FR-007**: clerk MUST surface merge conflicts: if `run_update` leaves conflict
  markers (or `.rej` files), clerk MUST report the affected paths and exit 4.
- **FR-008**: clerk MUST support `--vcs-ref` to target a specific version rather
  than the latest tag.
- **FR-009**: clerk MUST add `run_update` glue to `runner.py` (`update` and
...
- **FR-012**: clerk MUST refuse to downgrade (target version < current `_commit`
  version); exit 1 with a clear message.

### Key Entities

- **Upgrade announcement**: the per-layer `from_version → to_version` line clerk
...
- **Conflict**: one or more files with inline markers (or `.rej` files) left by
  copier's 3-way merge; clerk reports them; user resolves.
...
- **Q-006a — Conflict UX**: when `run_update` leaves conflict markers in files,
  clerk needs to report them. **How should this be detected?** copier does not
...
- **Q-006b — DAG re-resolution for new deps**: when the target template version
  ...
  should clerk do? Options: (i) **warn and continue**: upgrade the layers that exist,
...
- **Q-006c — `skip_tasks` and migrations**: copier's `run_update` accepts
  ...
  update path vs the `_tasks` path). If they are separate, clerk should expose them
...
- **Q-006e — `conflict` parameter surface**: `run_update` accepts `conflict=
  'inline'` (default) or `'rej'`. Should clerk expose this as a flag? Initial
...
- **Spec 010** (delivery contract: `scripts/clerk.py update` verb; N=1 uniform loop).
