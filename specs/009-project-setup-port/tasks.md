...
- [ ] T001 Verify Phase-0 source manifests before porting (Assumptions caveat): read
...
- [ ] T002 Scaffold base: `just new-module name=clerk-mod-base`. Confirm it creates
...
- [ ] T003 [P] Scaffold python: `just new-module name=clerk-mod-python`. Same
  confirmation for `templates/clerk-mod-python/`.
- [ ] T004 Run `just check-modules` — both fresh stubs MUST report `ok` (three-way
...
- [ ] T005 [US1] Author `clerk-mod-base` identity + choice questions in
...
- [ ] T006 [US1] Author `clerk-mod-base` gate + task-control booleans (Q5 / FR-007a):
...
- [ ] T007 [US1] Declare `clerk-mod-base` edges as `when:false` hidden answers with
  ...
  cross-module edges).
- [ ] T008 [P] [US2] Author `clerk-mod-python` questions in
...
- [ ] T009 [US2] Declare `clerk-mod-python` edge in `copier.yml` as a `when:false` hidden
...
- [ ] T010 Confirm both answers-file `.jinja` files exist and render the copier answers
...
- [ ] T011 [P] [US1] Author the **managed dir scaffold**: create
...
- [ ] T012 [US1] Author `templates/clerk-mod-base/template/AGENTS.md.jinja` (SEED-ONCE):
...
- [ ] T013 [US1] Add the base `_tasks` block to `copier.yml` in the plan's order
...
- [ ] T014 [US1] Add `_skip_if_exists: ["AGENTS.md"]` to `clerk-mod-base/copier.yml`
...
- [ ] T015 [US1] Author `clerk-mod-base` README prerequisites section documenting
  `git` + `gh` (authenticated) + `gitnr` (FR-007b) and the seed-once/managed lifecycle.
- [ ] T016 [P] [US1] Add the `clerk-mod-base` fixture to `tests/conftest.py` reusing
...
- [ ] T017 [P] [US1] `tests/loop/test_base_render.py` (US1 #1, SC-001): init
...
- [ ] T018 [US1] `tests/loop/test_base_trust.py` (US1 #2, SC-004): init an UNTRUSTED
...
- [ ] T019 [US2] Author `templates/clerk-mod-python/template/pyproject.toml.jinja`
...
- [ ] T020 [US2] Add `_skip_if_exists: ["pyproject.toml"]` to
  `clerk-mod-python/copier.yml` (FR-005a — language manifest is seed-once).
- [ ] T021 [US2] Add the `clerk-mod-python` **uv preflight `_task`** ordered FIRST
  (FR-007b / Q6): checks `uv` on PATH, fails with the astral install URL. Document `uv` in
  the module README.
- [ ] T022 [US2] Thread the Python `.gitignore` contribution through base's
...
- [ ] T023 [P] [US2] Add the `clerk-mod-python` fixture to `tests/conftest.py`; extend
  `multi_template_set` to build the ordered `[clerk-mod-base, clerk-mod-python]` pair with
  the `run_after` edge.
- [ ] T024 [US2] `tests/loop/test_python_overlay.py` (US2 #1, SC-002): init
...
- [ ] T025 [P] [US2] `tests/loop/test_python_standalone.py` (US2 #2, SC-006-style):
...
- [ ] T026 [US3] `tests/loop/test_base_python_reproduce.py` (US3 #1, SC-003): generate
...
- [ ] T027 [US3] Extend the reproduce test (US3 #2): assert trust-gated tasks
...
- [ ] T028 [US4] `tests/loop/test_arch_replay.py` (US4 #1, SC-005): init base with
...
- [ ] T029 [US4] `tests/loop/test_seed_once.py` (US4 #2, SC-003a): generate `[base,
...
- [ ] T030 [US5] Run `just check-modules` on the finished modules → MUST report `ok`:
...
- [ ] T031 [P] [US5] Add the spec-005 secrets-policy assertion coverage: confirm neither
...
- [ ] T032 [P] [US5] Update `skills/clerk/SKILL.md` (FR-012): document the ported family
  ...
  identity root.
- [ ] T033 [US5] Confirm the Phase-0 slice unblocks 008b (SC-008): with ≥1 real module
...
- [ ] T034 [P] Optional sanity: `just lint` / `uv run mypy` (docs/config only — not
  required by the plan). Confirm no `jinja2_time` / random filters in either `copier.yml`
  (Constitution V).
- [ ] T035 Final review against FR-001..013 + SC-001..008; record any residual ambiguity
