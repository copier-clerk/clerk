# Feature Specification: project-setup module port → bailiff-mod-* templates (spec 009)
...
collapsed `bailiff-mod-base` + `bailiff-mod-python`, agentic category excluded). **Amended
...
tooling/config choices instead of baking one opinion, and `bailiff-mod-ci` gains multiple
...
008 (packaging). Delivers the first real `bailiff-mod-*` modules, which **unblocks** the
...
(`bailiff-mod-*`) driven by bailiff's existing discover/init/reproduce/ordering machinery
...
model, its own gate/consent system, and its own catalog. bailiff already owns
...
code-executing steps — with NO new bailiff tool code (Constitution I / C-11).
...
- Q8: Faithful translation vs de-opinionated generalization? → A: **De-opinionate.**
  ...
  modern default, and honour every other bailiff constraint (no `secret:` questions;
  language manifests seed-once; code-executing steps trust-gated; deterministic
  renders). Applies to the **already-built** modules too (`bailiff-mod-base`,
  `bailiff-mod-python`, `bailiff-mod-apm` are revised, not left as-is).
- Q9: CI module scope — one sized workflow vs multiple optimization models? → A:
  **Multiple agent-selectable CI models.** `bailiff-mod-ci` MUST offer a finite,
...
- Q: Base split — one `bailiff-mod-base` vs several separate repos (OQ-009-a) → A:
  **One collapsed `bailiff-mod-base`.** The 6 base modules (core-identity,
...
- Q: Phasing / scope of v1 (OQ-009-b) → A: **Phase 0 = `bailiff-mod-base` +
  `bailiff-mod-python`** is v1 (the first merged slice) — the minimal real module set
...
- Q: Reproduce determinism for `agent`-tier modules + file lifecycle (OQ-009-c) → A:
  ...
  **(2) Every module classifies its outputs into two lifecycles:** **managed** files
  (bailiff owns; re-rendered config-consistently at reproduce) vs **seed-once / living**
  files (scaffolded at init, then OWNED and evolved by the project — bailiff must not
  ...
  an already-populated tree. The per-file behaviour under `bailiff update` (never-merge
...
- Q: Which project-setup `agentic` modules does 009 port? (OQ-009-d, 007 boundary) →
  ...
  = 007's `bailiff-mod-apm` (v1); `mcp-config`/`speckit-bridge` = future 007-family
  ...
  port-`codex-config`).
- Q: How do project-setup gates map onto bailiff? (OQ-009-e) → A: **Consequential
...
- Q: Multichoice options — fixed `choices:` vs runtime injection (OQ-009-g) → A:
  ...
  **generated by the `gitnr` tool from the project baseline** (the stack/language
  ...
  (process-deterministic, like the LICENSE fetch), not a config-rendered file. No runtime
...
2. **Decide the base split** — one collapsed `bailiff-mod-base` versus several separate
   `bailiff-mod-*` repos for the 6 base modules (the roadmap flags both; HIGH-priority
   open question).
...
5. **Map the tier model faithfully**: project-setup `python` steps → copier render;
   ...
   bailiff's trust gate + copier `when:` conditionals; code-executing steps →
   trust-gated `_tasks`.
...
The entire port is delivered as copier template content authored in the bailiff monorepo
under `templates/bailiff-mod-*/`, scaffolded via `just new-module` and linted by
`just check-modules` (the 008b authoring tooling). NO new `src/bailiff/` module and NO new
`scripts/bailiff.py` verb is introduced. The existing `init_many`/`reproduce_many` engine
...
project-setup already encodes, per module, exactly the metadata bailiff needs. The port is
a field-by-field translation:

| project-setup `module.toml`            | bailiff `copier.yml`                                          |
|----------------------------------------|-------------------------------------------------------------|
...
| `gate` (`hardness`, `allow_flag`)      | bailiff trust gate + copier `when:` conditional (see OQ-009-e) |
...
freezes stack pins). In bailiff's two-phase model (Constitution II) these are **phase-1
...
`_tasks`, gated by bailiff's trust mechanism exactly as `bailiff-template-example`'s
...
config-consistent (Constitution III) — pin versions in the task command where possible.
...
- `git-init`'s commit step declares soft `after` edges over **every** other module (it
  must commit last) — in bailiff this is a `run_after`-all / `run_before: []`-tail edge;
...
Per 007's clarify, `bailiff-mod-apm` (spec 007) is shipped **independently**; it is NOT a
...
re-implement 007's `bailiff-mod-apm`. **Resolved (Q4/OQ-009-d): 009 excludes the ENTIRE
`agentic` category** (`apm-install`, `mcp-config`, `speckit-bridge`, `codex-config`).
007's family owns all agentic wiring — `apm-install` is 007's `bailiff-mod-apm` (v1);
...
Not every file a module writes should be owned by bailiff forever. Each module MUST
classify its outputs into two lifecycles:

- **Managed** — bailiff owns the file; reproduce re-renders it config-consistently from the
  committed answers (Constitution III). The default for pure scaffold/config.
- **Seed-once / living** — bailiff scaffolds the file at init, then the **project owns and
  evolves it**; bailiff MUST NOT clobber it on a re-run. These are files that legitimately
...
of destination paths never overwritten once present) — no new bailiff tool code (C-11).
...
under `bailiff update` (never re-touch vs copier's 3-way merge) is a plan-level, per-module
classification.
...
- **New behavioural capabilities not present in project-setup** (roadmap Scope-out). No
  ...
  **NOT excluded (amended Q8/FR-011/FR-014):** de-opinionating a hardcoded decision into a
  ...
  and `bailiff-mod-ci`'s multiple agent-selectable CI models (Q9/FR-015) — these are
  sanctioned generalizations, not new capability.
- **The fan-out / release pipeline** (spec 008b) — 009 authors module *content* into
  `templates/bailiff-mod-*/`; 008b's CI fans them out. 009 unblocks 008b by existing; it
...
- **spec 007's `bailiff-mod-apm` and the whole `agentic` category** — independent; 009
...
- **Porting project-setup's runner, SDK, or manifest format** — those are the mechanisms
  bailiff replaces with copier + the spec-003 DAG. The runner does not ship; only the
  modules' behaviour is re-expressed as templates.
- **project-setup's home-config catalog / `sources.toml` / `answers.toml` machinery** —
  bailiff's spec-002 catalog + copier answers files supersede these.
...
A developer selects the collapsed bailiff base (`bailiff-mod-base`, Q1) with a
`project_name`, `org`, and `license`; bailiff applies it; the generated project contains
...
1. **Given** a base selection with `project_name=demo`, `org=acme`, `license=MIT`,
   **When** `init`, **Then** the generated project contains the scaffold + a
   ...
   copier alone (no agent, no bailiff).
2. **Given** the same selection with the source UNTRUSTED, **When** `init`, **Then**
   bailiff refuses at exit 3 (naming the `trust add` command) before running the
   `git init`/LICENSE tasks.

### US2 — Add a language overlay on top of base (Priority: P1)

A developer selects `[bailiff-mod-base, bailiff-mod-python]`; the spec-003 engine applies
...
is config-consistent (same recorded answers + pinned commit), and trust-gated tasks re-run.

**Acceptance Scenarios**:
1. **Given** a generated `[base, lang-python]` project, **When** `reproduce`, **Then**
   all rendered files are config-consistent and the reproduce order is recomputed from the
...
1. **Given** agent-resolved structured facts (stack, env-keys, pins) frozen as answers,
   **When** `reproduce`, **Then** the derived file (README/STACK/env) re-renders from
   those frozen facts config-consistently, with no agent call.
...
Each module authored under `templates/bailiff-mod-*/` passes `just check-modules`
...
- **Base split changes edge identity**: if the 6 base modules collapse into one
  `bailiff-mod-base`, the fine-grained `requires`/`after` edges between them disappear
...
- **Multichoice / list options** (Q7/OQ-009-g): finite well-known sets (SPDX licenses,
  ...
  config-rendered file. No runtime `--data` injection is needed in 009 (the dynamic lists
  were the agentic category, excluded by Q4).
- **Secret-shaped answers**: project-setup enforces a G8 secret-shape guard at its
  interview boundary. bailiff's equivalent is Constitution VI (no `secret:` questions) +
...
- **`license-write`'s dynamic fetch**: like `bailiff-template-example`, the LICENSE is a
  task side effect outside the config-consistent reproduce set (network-sourced) — port it
  the same way (guarded, idempotent task).
- **Overwrite / drift on re-run**: project-setup hard-gates destructive overwrites;
  bailiff's `reproduce` uses `--overwrite` (faithful replay) and `update` is the explicit
...
- **FR-002**: No shipped module MAY declare a `secret:` question (Constitution VI
  ...
  (as `bailiff-template-example`'s LICENSE task reads `gh` auth), never a copier answer.
...
- **FR-005**: Each module's deterministic (`python`-tier) **managed** outputs MUST be
  produced by rendered template files, config-consistent across reproduce runs
...
- **FR-007b** *(tool preflight — Q6/OQ-009-f)*: A module whose task requires an external
  ...
  literally pre-render.) No `[tools]`-equivalent bailiff glue is added (C-11).
- **FR-007c** *(option lists — Q7/OQ-009-g)*: Finite well-known option lists (SPDX
  ...
  (process-deterministic), not a config-rendered managed file.
...
- **FR-014** *(de-opinionation — Q8, 2026-07-14)*: Each module MUST NOT force a
  ...
  only rendered files / task commands / edges — no new `src/bailiff/` code (C-11). This
  requirement applies to the already-built `bailiff-mod-base`, `bailiff-mod-python`, and
  `bailiff-mod-apm` (they are REVISED to satisfy it), not only to newly authored modules.
- **FR-015** *(multi-model CI — Q9, 2026-07-14)*: `bailiff-mod-ci` MUST offer a finite,
...
- **FR-013** *(base split — RESOLVED, Q1/OQ-009-a)*: The 6 base modules MUST ship as ONE
  collapsed `bailiff-mod-base` copier template. Their inter-base ordering (identity →
  dirs/gitignore → license → agents-md → git-init commits last) is expressed
  template-internally, NOT as cross-module `when:false` edges. `bailiff-mod-base` is a
  single fan-out target with one answers file. Later layers (languages, quality, etc.)
  `run_after` `bailiff-mod-base` as a whole.

### Key Entities

- **`bailiff-mod-*` template**: one ported module — a copier template in the monorepo under
  `templates/bailiff-mod-<name>/`, fanned out to `bailiff-io/bailiff-mod-<name>` by 008b.
- **`bailiff-mod-base`**: the single collapsed base scaffold template (core identity +
...
- **Language overlays** (`bailiff-mod-python`/`-ts`/`-go`/`-rust`): stack tooling layers
  that `run_after` base.
...
|---|---|---|
| **base** (always-on) | core-identity, dirs-scaffold, gitignore-generate, license-write, agents-md, git-init | Collapsed into ONE `bailiff-mod-base` (Q1/OQ-009-a). `agents-md` has an `agent` step + a hard gate and its `AGENTS.md` is seed-once (`_skip_if_exists`, D-009-7); `gitignore-generate` shells out to `gitnr` (pinned trust-gated task, Q7); `git-init`/`license-write` are tasks. |
...
- **SC-001**: A generated base project contains the correct scaffold (dirs, `.gitignore`,
  ...
  reproduce config-consistently with copier alone; `.gitignore` (gitnr) and `LICENSE`
...
- **SC-003**: Reproduce of any ported project onto a fresh checkout re-renders all
  **managed** files config-consistently and re-runs trust-gated tasks; the order is
...
- spec 007's `bailiff-mod-apm` (independent) and the entire project-setup `agentic`
...
that exists is `examples/bailiff-template-example/` (a demo, not a shippable module). 009 is
the linchpin — the moment it lands ≥1 real `bailiff-mod-*` module under `templates/`, 008b
...
**009 ↔ 007 (independent; agentic category is 007's):** 007 ships `bailiff-mod-apm`
...
- **Phase 0 — Contract slice = v1 (unblocks 008b):** port the collapsed `bailiff-mod-base`
  (Q1) + one language overlay (`bailiff-mod-python`). This is the minimal real module set
...
- **Phase 1 — Remaining languages + quality/tooling (pure `python`-tier):**
  `bailiff-mod-ts`, `bailiff-mod-go`, `bailiff-mod-rust`, `bailiff-mod-precommit`,
  `bailiff-mod-quality`, `bailiff-mod-justfile`. Mostly deterministic renders; lowest
...
- **Phase 2 — Agent-steered + integration + monorepo + CI (the hard translations):**
  `bailiff-mod-env`, `bailiff-mod-readme`, `bailiff-mod-stack-adr` (frozen-answer determinism
  — OQ-009-c), `bailiff-mod-ci` (multi-model, Q9/FR-015), `bailiff-mod-github-repo`,
  `bailiff-mod-package-add` (monorepo), and last `bailiff-mod-org-policy` (a no-op until an
  org-source-fetch module exists — flagged). `bailiff-mod-codex` (codex-config) is ported
...
- **Dropped:** `worktreeinclude-write` (`bailiff-mod-worktree`) — niche, low-value, awkward
...
### OQ-009-a — Base split: one `bailiff-mod-base` vs several separate repos (HIGH PRIORITY)  [RESOLVED — Q1, 2026-07-13]

**Resolution**: (A) one collapsed `bailiff-mod-base`. The 6 base modules ship as one
...
`bailiff-mod-base` copier template, or ship as several separate `bailiff-mod-*` repos? The
roadmap explicitly flags both ("a real bailiff-mod-base could collapse 5 project-setup base
modules; 009 may instead ship them as separate bailiff-mod-* repos"). This is roadmap
open-question **Q2** ("keep `bailiff-mod-base` as one template or re-split?"), which the
roadmap says is "resolved when 009 is scoped" — i.e. here.

**Tradeoffs**:
- **(A) One collapsed `bailiff-mod-base`**: one repo, one tag line, one catalog entry, one
...
- **(B) Several separate base modules**: each of the 6 is its own `bailiff-mod-*` repo. Maps
...
**Lean (flagged for review)**: **(A) one collapsed `bailiff-mod-base`** — project-setup
...
**Resolution**: v1 = Phase 0 (`bailiff-mod-base` + `bailiff-mod-python`). Unblocks 008b
...
**Lean**: Phase 0 (base + `bailiff-mod-python`) as the first merged slice (unblocks 008b),
...
config-consistently) vs **seed-once/living** (scaffolded once, then project-owned):
...
FR-005a, FR-006. The per-file `bailiff update` merge policy is a plan-level detail.
...
In bailiff's model that judgment MUST happen in phase 1 and freeze into a `--data` answer
...
(c) reproduce replays config-consistently with no agent?
...
`speckit-bridge`, `codex-config`) overlaps spec 007's `bailiff-mod-apm`. Does 009 (a) port
...
### OQ-009-e — How do project-setup gates map onto bailiff?  [RESOLVED — Q5, 2026-07-13]
...
`allow-install`, `allow-stack-write`). bailiff has a coarser model: one trust gate (source
trusted → `_tasks` run) + copier `when:` conditionals for opt-in/opt-out. Do the
fine-grained per-action consents map onto bailiff, or collapse into "the source is trusted
and the user selected the module"?

**Tradeoffs**: bailiff's trust gate is all-or-nothing per source; it does not have
...
bailiff's simpler, source-level trust model.
...
bailiff's model without inventing a per-action consent layer (which would be new tool code —
...
bailiff glue (C-11). Options retained below for context.
...
(bailiff's `_translate` reports it) — simplest, but the failure is late (after render).
...
that bailiff already has `bailiff doctor` (spec 008) for bailiff's OWN deps — not the
generated project's, so it does not cover this.

**Lean**: **(i)+(ii)** — rely on the task's own loud failure + a documented prerequisite,
consistent with how `bailiff-template-example` requires `gh` (its task fails if `gh` is
...
a config-rendered file). No runtime `--data` injection in 009 (dynamic lists were the
...
- The 008b authoring tooling (`just new-module`, `scripts/check_modules.py`,
  ...
  009 module; 009 modules land under `templates/bailiff-mod-*/`.
...
- **Constitution III** (faithful, agent-free reproduce: rendered files config-consistent;
...
- **ADR-0003** (`when:false` dependency edges statically read; `--data` runtime injection
  in scope from question 1; bailiff threads answers between layers via `data=`).
