...
- Q: Reproduce determinism for `agent`-tier modules + file lifecycle (OQ-009-c) → A:
  ...
  **(2) Every module classifies its outputs into two lifecycles:** **managed** files
  (bailiff owns; re-rendered byte-identically at reproduce) vs **seed-once / living**
...
- **Managed** — bailiff owns the file; reproduce re-renders it byte-identically from the
