# Implementation Plan: project-setup module port → clerk-mod-* (spec 009) — **Phase 0**
...
> **`clerk-mod-base`** (6 base modules) + one language overlay **`clerk-mod-python`**.
...
> fan-out/release/e2e phases are hard-blocked until ≥1 real `clerk-mod-*` module lands
> under `templates/`).
...
under `templates/clerk-mod-base/` and `templates/clerk-mod-python/`. The port is a
...
no new `src/clerk/` module, no new `scripts/clerk.py` verb.
...
`clerk-template-example` today), `gitnr` (pinned, `.gitignore` generation — Q7), and for
`clerk-mod-python` the `uv` toolchain surfaced by preflight.
...
**Scale/Scope**: 2 modules. `clerk-mod-base` collapses 6 project-setup base modules into
one template (Q1/FR-013); `clerk-mod-python` is one language overlay `run_after` base.
...
|---|---|---|
| **I — Skills + Templates + Minimal Glue (C-11)** | PASS | Pure template content under `templates/clerk-mod-{base,python}/`. NO new `src/clerk/` module, NO new `scripts/clerk.py` verb. Q5 (gates→copier booleans + source trust), Q6 (preflight `_task`), Q7 (gitnr `_task`) each avoid new glue — verified no copier gap. Authored via existing `just new-module`; linted by existing `check_modules.py`. |
...
| **VI — Template-author contract** | PASS | Both modules ship `{{ _copier_conf.answers_file }}.jinja`; clean PEP 440 tags via cocogitto fan-out (ADR-0006); `when:false` edges; new `_migrations` format only if any declared (none in Phase 0); **NO `secret:` questions** — LICENSE/`gh` reads token from ambient env (like `clerk-template-example`). |
...
```text
templates/
├── clerk-mod-base/                         # Q1: 6 base modules collapsed into ONE template
...
└── clerk-mod-python/                       # language overlay, run_after clerk-mod-base
...
cog.toml                 # [monorepo.packages.clerk-mod-base] + [.clerk-mod-python]
catalog-sources.toml     # [[sources]] url for each (created on first new-module run)
skills/clerk/SKILL.md    # FR-012: documents the ported family + base-selection step
```

**Structure Decision**: mirror `examples/clerk-template-example/` (a `_subdirectory:
...
fans each out to `copier-clerk/clerk-mod-<name>` (consumers source the split repo, never
...
byte-identical set — like `clerk-template-example`'s LICENSE).

### `clerk-mod-base` (collapses core-identity, dirs-scaffold, gitignore-generate, license-write, agents-md, git-init)
...
| `LICENSE` | license-write | **task-output** (gh api) | Ported exactly like `clerk-template-example`: `test -f LICENSE || gh api /licenses/<key>` with `[year]`/`[fullname]` filled from frozen `today`/`org`. Network-sourced → outside the byte-identical set; guard makes reproduce idempotent. `license` is a fixed 13-SPDX `choices:` question (FR-003 / Q7). |
...
**`clerk-mod-base` — no cross-module edges** (it is the upstream root). Its
...
**`clerk-mod-python` — one edge**: `run_after: [clerk-mod-base]` as a `when:false`
...
`default: "{{ project_name }}"` and clerk threads it via `data=` (FR-010); the overlay
...
(base + python), clerk injects the selected language(s) into `gitignore_stack` via
...
- **Q1 — collapsed base (FR-013)**: ONE `templates/clerk-mod-base/` template, one
...
- **Q6 — tool preflight**: each module ships a preflight `_task` ordered FIRST that
  ...
  (`clerk-mod-base`: git/gh/gitnr; `clerk-mod-python`: uv). Plus a README prerequisite
...
- **Phase 1** — remaining pure-`python`-tier overlays + quality/tooling: `lang-ts`,
  ...
  clerk-mod-python's deferred ruff-hook contribution.)
