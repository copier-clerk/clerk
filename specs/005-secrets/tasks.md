---
description: "Task list for bailiff secrets policy — no scaffold-time secrets; agent never handles credentials (spec 005)"
---

# Tasks: bailiff secrets policy — no scaffold-time secrets; agent never handles credentials
...
- 005 is a **policy + guardrail**, NOT a secrets engine. No `src/bailiff/` module added;
  `discovery.secret_questions` (slice 001) is the whole detection mechanism.
- Lint home (Q-005a) = a small standalone policy test now over in-repo bailiff-authored
...
- Third-party non-interactive (Q-005b) = fail loud naming the question (decision 4b /
  FR-003c); bailiff never prompts (Constitution V) and does NOT render copier's
...
- [X] T004a [US2] `src/bailiff/discovery.py` (FR-003b): populate `secret_questions` from BOTH per-question `secret: true` AND the top-level `_secret_questions: [keys]` list form (copier honors both — verified). Add the list-form parse after the per-question loop; dedupe. So bailiff's flag set matches copier's own exclusion set.
- [X] T004b [US2] `src/bailiff/errors.py`: add `SecretInAnswersError(BailiffError)` — carries the offending KEY name(s), NEVER the value; message names the key + explains secrets are supplied out-of-band, not in the run-spec.
- [X] T004c [US2] `src/bailiff/runner.py` (FR-003a): in `init` AND `init_many`, before calling copier, compute `set(answers) & secret_questions` for each layer's source (via `discovery`); if non-empty, raise `SecretInAnswersError(keys)` — fail loud, non-zero exit, on BOTH paths. This is the enforced boundary behind the SKILL rule.
- [X] T004d [US2] `src/bailiff/runner.py` (FR-004): redact secret answer values before wrapping/surfacing copier errors — the current `raise InvalidRunSpecError(f"...{exc}")` at ~line 146 (and the multi-layer error paths) can echo a `validator`-carried secret. For runs involving secret keys, use a generic message / scrub the value from `{exc}` before re-raising.
- [X] T004e [US2] `src/bailiff/runner.py` (FR-003c): for a required secret question with no value supplied in a non-interactive run, fail loud naming the question rather than proceeding on copier's placeholder default.
...
- [X] T005 [US3] Extend `tests/loop/test_secret_edge_exclusion.py`: keep the non-persistence assertion; ADD assertions that when a secret value IS supplied at the deterministic step (programmatic `run_copy(data=…)`), the value does NOT appear in (a) `.copier-answers.yml`, (b) captured stdout/stderr, (c) `--pretend`/`--check` output. Confirm bailiff never builds a `copier --data key=value` argv for a secret.
- [X] T006 [P] [US3] (Optional, demonstrates the pattern) Add `.env.example.jinja` + README guidance to `examples/bailiff-template-example/` showing the runtime-secret pattern — the generated project owns its secrets at runtime, NOT via copier answers. Keep it minimal; it doubles as living documentation for SC-005.
...
- **Phase 1 (T001–T002)** is 003-independent — the policy lint on bailiff-authored
...
- SC-001 — bailiff-authored template with a secret question fails the policy lint
  (T001).
