# Feature Specification: clerk engine — PyPI packaging, multi-catalog, listing cache, collision check, capability tags (spec 013)
...
`clerk capabilities` verb) in favor of a slim warn-only design. All design decisions below are
maintainer-ratified fixed inputs; only the items explicitly marked NEEDS CLARIFICATION remain
open.

**Input**: Maintainer-ratified decisions of 2026-07-14 (as adjudicated): publish `src/clerk/`
...
clerk today is deliberately *not* a published application. Constitution I (v2.3.0) states there
is "no `[project.scripts] clerk` console entry, no `uvx clerk` PyPI tool", and C-11
...
**clerk as the tool** (installed via `uvx`, published to PyPI) and **`clerk-mod-*` as the
...
Scope is **engine work only**: packaging/publication of `src/clerk` as a CLI, informational
...
e.g. two monorepo-tool modules, or two docs-site engines. `clerk init` emits a LOUD WARNING
...
clerk does not enforce claims. (Upgrading warn→error later is a trivial change if real-world
...
`_clerk_provides: [monorepo-tool]`, one of which also declares `_clerk_exclusive: true` →
`clerk init --run-spec` prints a loud warning naming `monorepo-tool` and both members, exits
...
1. **Given** a selection containing two providers of capability `C`, where at least one
   provider of `C` anywhere in the merged listing declares `_clerk_exclusive: true`, **When**
   `clerk init --run-spec` runs, **Then** a loud warning is emitted before rendering, naming
...
2. **Given** the same conflict where the two SELECTED modules did not themselves declare
   `_clerk_exclusive` but a third provider of the same capability in the merged listing did,
...
A module author adds an optional key — `_clerk_provides: [<capability>, …]` — to their module's
...
`_clerk_exclusive: true` — a self-referential statement ("my capability is a pick-one slot")
that never names sibling modules. First-party CI (`check_modules.py`) lints only
well-formedness (list of kebab-case strings; `_clerk_exclusive` boolean) and flags MIXED
...
to `check_modules(templates_dir=…)`), add `_clerk_provides: [hook-manager]` to its
`copier.yml` → `clerk discover` on that template reports the capability list;
...
declaring `_clerk_exclusive: true` → `check_modules.py` flags the mixed group.

**Acceptance Scenarios**:

1. **Given** a module whose `copier.yml` contains `_clerk_provides: [<capability>, …]`,
   **When** the module is discovered, **Then** the capability list appears in the discovery
   output (and therefore in `clerk discover`'s dict) without executing any template code.
2. **Given** the same module, **When** the repo catalog is regenerated, **Then** its
   `catalog.json` entry carries the capability list (and `_clerk_exclusive` when declared)
   alongside name/description/source/tags.
3. **Given** a first-party module whose `_clerk_provides` is not a list of kebab-case strings
   (or whose `_clerk_exclusive` is not a boolean), **When** `check_modules.py` runs, **Then**
...
4. **Given** two first-party modules sharing a capability where only one declares
   `_clerk_exclusive: true`, **When** `check_modules.py` runs, **Then** the mixed group is
   flagged as an author-time error (all siblings of a pick-one family should declare it).
5. **Given** a module with no `_clerk_provides` key, **When** discovered/validated, **Then**
   it is treated as unconstrained (no group membership, no warnings).

---

### User Story 3 — Install and run clerk via uvx (Priority: P1)

A user with no clerk checkout runs `uvx <dist-name> init …` (or installs the package and runs
`clerk init` / `clerk reproduce`). They get the full CLI — every verb the bundled script offers
...
**Why this priority**: This is the repositioning itself — clerk as the tool. It is also the
scope that requires the constitution amendment, so it is the release gate for the whole spec.

**Independent Test**: `uv build` the wheel, install it into a clean venv → the `clerk` console
command exists, `clerk --version` matches `pyproject.toml`, `clerk doctor` passes, and a
...
2. **Given** the installed package, **When** `--version` is queried, **Then** it reports the
   ...
   `pyproject.toml` and `clerk.__version__`).
3. **Given** the repo checkout WITHOUT an installed distribution (the bundled
   `scripts/clerk.py` path), **When** `--version` is queried, **Then** `clerk.__version__`
...
not either declares any capability. `clerk init` detects the overlap BEFORE any file is written
...
destination path → `clerk init --run-spec` fails with a typed `ClerkError` (exit 1) naming the
...
1. **Given** two selected modules that would each write the same managed destination path,
   **When** `clerk init --run-spec` runs, **Then** a typed error in the `ClerkError` hierarchy
...
clerk pointer second (the existing ordered `[[catalog]]` tables). Their internal fork of
`clerk-mod-python` (same module name) resolves first for bare-name selection — deliberately,
with a loud shadow warning naming both sources so nobody is surprised. The shadowed public
entry remains fully addressable by full-id (`public/clerk-mod-python`) and remains visible in
...
`clerk-mod-python` → selecting the bare name `clerk-mod-python` resolves to the first pointer's
entry with a loud shadow warning naming the shadowed source (today this raises the ambiguity
`CatalogError`, catalog.py:455-466); `clerk catalog list` shows BOTH entries with the second
...
3. **Given** the same configuration, **When** `clerk catalog list` runs, **Then** BOTH entries
...
4. **Given** an existing single-pointer `catalog.toml` written by a pre-013 clerk, **When**
...
A user (or agent) runs `clerk catalog refresh` once; clerk builds the full listing (this is the
step that clones/discovers every source) and persists it. Subsequent `clerk catalog list`
calls, selection validation, and capability-tag reads consume the persisted listing — fast, and
with no per-source network traffic. When no cache exists, clerk instructs the user to refresh
...
**Independent Test**: Run `clerk catalog refresh` → a listing file appears under the
platformdirs cache dir; run `clerk catalog list` twice → byte-identical output with zero
network/git traffic; delete the cache → `clerk catalog list` either instructs the user to run
refresh or transparently builds the cache once (single defined behavior, see FR-016).

**Acceptance Scenarios**:

1. **Given** a configured catalog, **When** `clerk catalog refresh` runs, **Then** the full
   listing (templates, unusable bucket, capability tags, shadow marks) is built and persisted
   in the platformdirs cache location.
2. **Given** a persisted listing, **When** `clerk catalog list` (including `--json`) or
...
- **Module description naming a sibling** ("use this instead of clerk-mod-X"): forbidden
  authoring practice — `_clerk_exclusive` is self-referential precisely so descriptions never
  go stale against the sibling set.
- **Incremental add into a non-clerk directory** (no `.copier-answers.*.yml` present): the
...
- **`clerk catalog list` with an empty catalog**: prints an empty listing, exit 0 (existing
  behavior, unchanged).
...
- **PyPI name `clerk` is taken** (verified 2026-07-14: an existing unrelated `clerk 0.1.0`
  package occupies it): plain `uvx clerk` is impossible; the distribution name decision is
  flagged below.
...
- **FR-001** *(console entry point)*: The CLI currently living in `scripts/clerk.py` MUST be
  available from the installed package: its verb dispatch moves (or is re-exported) into
  `src/clerk/` (e.g. `clerk/cli.py`) and `pyproject.toml` gains a `[project.scripts]` entry
  ...
  with the existing exit-code contract preserved (0 ok, 1 `ClerkError`, 2 usage, 3
...
- **FR-002** *(dependency correctness)*: `platformdirs` — imported directly by
  `src/clerk/catalog.py`, `src/clerk/trust.py`, and `src/clerk/defaults.py` but today only
...
- **FR-003** *(single-source version)*: The version MUST be single-sourced: `pyproject.toml`'s
  `version` and `clerk.__version__` MUST NOT be independently maintained duplicates; the
  release flow (cog-tagged) MUST keep the published version, the tag, and `--version` output
  in agreement. The mechanism MUST keep `clerk.__version__` resolvable WITHOUT installed
  distribution metadata (e.g. a guarded `importlib.metadata` lookup with a fallback, or hatch
  dynamic versioning reading from the source) — the bundled `scripts/clerk.py` path (FR-006)
  imports `clerk.__version__` from a bare checkout and MUST keep working.
...
- **FR-005** *(distribution name)*: The PyPI name `clerk` is TAKEN (verified 2026-07-14
  against pypi.org: an unrelated `clerk 0.1.0` exists); `copier-clerk` (the current
  `pyproject.toml` name) and `clerk-scaffold` were both available at verification time. The
  spec REQUIRES: (a) the console command is `clerk` regardless of distribution name; (b) the
  ...
  [NEEDS CLARIFICATION: `copier-clerk` (status quo, invoked `uvx --from copier-clerk clerk`)
  vs `clerk-scaffold` (closer to `uvx clerk-…` ergonomics) — maintainer decision].
- **FR-006** *(bundled-script disposition)*: The skill-bundled `scripts/clerk.py` invocation
  path (`./scripts/clerk.py` / `uv run scripts/clerk.py`) MUST keep working for the clerk repo
...
- **FR-007** *(declaration)*: A module MAY declare `_clerk_provides: [<capability>, …]` in its
...
- **FR-008** *(exclusivity declaration)*: A module MAY additionally declare
  `_clerk_exclusive: true` — a SELF-REFERENTIAL statement meaning "my capability is a
...
- **FR-009** *(aggregation)*: Discovery MUST parse the declarations statically (never
  ...
  the user-catalog listing built for the cache (FR-016). `clerk catalog list` output MUST show
...
- **FR-010** *(conflict warning — never a block)*: At `init_many` selection time, when the
  ...
  FR-008's group-infection rule, clerk MUST emit a LOUD WARNING naming the capability and the
...
- **FR-011** *(first-party CI lint)*: `scripts/check_modules.py` MUST lint capability
  declarations for WELL-FORMEDNESS ONLY: `_clerk_provides` is a list of kebab-case strings,
  `_clerk_exclusive` is a boolean. It MUST additionally flag MIXED exclusivity declarations
...
- **FR-013** *(pre-render overlap scan — the hard backstop)*: `init_many` MUST perform a
  ...
  same managed destination path, init MUST fail with a typed error in the `ClerkError`
...
- **FR-014** *(bare-name precedence + compat)*: "Catalog" here means the EXISTING
  ...
  `CatalogError` (catalog.py:455-466). Full-ids (`internal/clerk-mod-x`) remain individually
...
- **FR-015** *(shadowed entries stay visible)*: On bare-name shadowing, the shadow warning
  ...
  `clerk catalog list` output, explicitly marked as shadowed — shadowing affects bare-name
  resolution, never listing visibility. There is NO content-based dedup — name precedence
  only.

### Functional Requirements — listing cache

- **FR-016** *(persisted listing + refresh)*: A new `clerk catalog refresh` action MUST build
  the full listing (templates, unusable bucket, capability tags, shadow marks) and persist it
  under the platformdirs cache directory. `clerk catalog list`, selection validation, and
...
- **FR-018** *(constitution amendment — release gate)*: Before ANY PyPI publish under this
  ...
  from spec 011: (1) amend Constitution I (which verbatim forbids a `[project.scripts] clerk`
  console entry and a `uvx clerk` PyPI tool) to permit the published engine, with a
  ...
  recording the repositioning (clerk = the tool, clerk-mod-* = the modules) and the tradeoff;
...
- **Capability declaration**: the optional `_clerk_provides: [<capability>, …]` list in a
...
- **Exclusivity flag**: the optional `_clerk_exclusive: true` self-referential declaration;
...
- **Persisted listing cache**: the `clerk catalog refresh` artifact under the platformdirs
  cache dir; the read source for list/selection/tag operations.
- **Collision error**: the new typed error in the `ClerkError` hierarchy (exit 1) raised by
...
- **Console entry point**: the `[project.scripts]` mapping making the packaged `src/clerk`
...
- **SC-001**: Installing the built wheel into a clean environment yields a working `clerk`
  console command: every existing verb runs with the documented exit-code contract,
  `clerk --version` matches the `pyproject.toml` version, and a declared-dependency audit
...
- **SC-005**: `generate_catalog.py` output for a declaring module carries its capability list
  ...
  first-party exclusivity group, reporting module + value; `clerk catalog list` shows the
  capability tags.
...
- **SC-007**: After `clerk catalog refresh`, `clerk catalog list` and selection validation
...
- PyPI name availability as verified 2026-07-14: `clerk` taken (unrelated `clerk 0.1.0`
  exists), `copier-clerk` and `clerk-scaffold` available. Availability is rechecked at
...
- The existing engine surface (`init_many`'s pre-check pattern, discovery's static
  ...
  `ClerkError` hierarchy, the 0/1/2/3/4 exit contract) is consumed as-is; 013 extends these
...
- 012 and 013 are decoupled: 012's modules MAY ship `_clerk_provides`/`_clerk_exclusive`
...
1. **Distribution name** (FR-005): `copier-clerk` (status quo; `uvx --from copier-clerk clerk`)
   vs `clerk-scaffold` — plain `clerk` is taken on PyPI (verified 2026-07-14). The console
   command is `clerk` either way.
2. **Bundled-script end-state** (FR-006): does `scripts/clerk.py` remain permanently as a thin
