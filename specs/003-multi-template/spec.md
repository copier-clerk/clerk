...
2. **Deterministic order via a documented stable tie-break.** Mutually-independent
   ...
   **stable, documented tie-break** (lexicographic by repo-basename) so the order —
   and thus the output for edge-independent layers writing disjoint paths — is
...
2. **Given** two edge-independent templates C and D writing disjoint paths, **When**
   init `[C, D]` and `[D, C]`, **Then** the rendered tree is config-consistent (order
...
twice yields config-consistent output, (c) the resolution used only the committed
...
2. **Given** the same project reproduced twice, **Then** the two outputs are
   config-consistent (deterministic recompute).
...
- **FR-006**: The recomputed reproduce MUST be deterministic (same committed state +
  pins → same order → config-consistent output for disjoint-writing layers) and
...
- **SC-002**: Multi-template reproduce is config-consistent across repeated runs and
...
- **SC-003**: Edge-independent layers writing disjoint paths produce config-consistent
