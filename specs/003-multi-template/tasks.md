...
- [ ] T011 [US2] `tests/loop/test_multi_reproduce.py` (NEW): from a US1-generated project — reproduce recomputes order respecting edges; reproduce TWICE → byte-identical (SC-002); assert resolution used only committed answers files + fetches (no recipe/DAG file present in dest); **copier-only-by-hand parity** — plain `copier recopy --vcs-ref=:current: -a <each file>` in the recomputed order yields the same tree (spec-010 fallback, N layers); **N=1 no-regression** — a single-template project reproduces identically through reproduce_many as through the 010 single path.
