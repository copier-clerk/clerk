# Implementation Plan: bailiff multi-template — dependency ordering + threaded init, recomputed reproduce
...
Build bailiff's **ordering brain** and slot it into the existing pieces: it consumes
...
New glue — a single `src/bailiff/ordering.py` module (the C-11-sanctioned coordination
code) — plus multi-layer entry points wired into `runner.py` and surfaced through
`scripts/bailiff.py`. Everything mechanical stays LLM-free (Constitution II).
...
(copier writes it; bailiff sets `answers_file=`). NO bailiff-authored order/recipe file
...
recomputed reproduce tested end-to-end + via subprocess through `scripts/bailiff.py`.
...
|---|---|---|
| I — Skills + templates + minimal glue | PASS | One new `ordering.py` module — THE sanctioned coordination glue (C-11: copier does zero cross-template coordination). Surfaced via existing `scripts/bailiff.py`; no new tool, no template, no adapter. |
...
| V — Determinism via pinning; trust by source | PASS | Order determinism from pins + stable tie-break. Trust unchanged: each layer's `copier copy` trust-gates via settings.yml; bailiff writes no trust. A layer needing tasks surfaces the existing UntrustedSourceError. |
...
└── errors.py            # EXTEND: OrderingError(BailiffError) — cycle / dangling edge / basename collision.

scripts/bailiff.py         # EXTEND: multi-template surface. Either extend `init`/`reproduce` to accept
...
skills/bailiff/SKILL.md    # EXTEND: after catalog selection (spec 002), document the multi-template
                         #   flow — validated selection → bailiff orders + applies layers → recomputed
...
**Structure Decision**: One new `src/bailiff/ordering.py` (pure functions: DAG build,
validation, topo-sort, layer naming), consumed by new `runner.init_many` /
`runner.reproduce_many`, surfaced through `scripts/bailiff.py`. Reuses
