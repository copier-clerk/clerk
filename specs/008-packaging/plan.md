# Implementation Plan: bailiff skill packaging — installable via APM marketplaces (Claude + Codex)
...
Make the bailiff skill installable into any project via APM, using APM's own tooling
(`apm pack` / `apm publish` / `apm marketplace`) — emitting **both** a Claude and a
Codex marketplace natively. Solve portability: the package **vendors `src/bailiff/*`**
(no PyPI `bailiff`, per spec 010) and **checks (does not auto-install)** its
...
`bailiff-mod-*` modules exist until spec 009).

Resolved planning decisions (flagged for review in spec Open Questions):
- **Vendoring** = copy `src/bailiff/*.py` into the package skill dir via a `just`
...
**Primary Dependencies**: no new *runtime* dep. The bailiff package's runtime deps
stay `copier>=9.16,<10`, `pyyaml`, `packaging`, `tomli-w` — now declared in a PEP 723
header on `scripts/bailiff.py` (single source of truth) AND checked by the preflight.
...
`apm.yml`). The vendored `src/bailiff` copy lives under the package skill dir
(`packages/bailiff/.apm/skills/bailiff/`). Nothing is written into a *consumer's* project by
packaging itself.
...
vendored-drift check (diff shipped `src/bailiff` copy vs source). `mypy --strict` +
...
**Constraints**: no PyPI `bailiff` (spec 010); no assumed package manager (preflight
...
`scripts/bailiff.py`; a PEP 723 header; a `just` vendoring + release recipe; a portable
...
|---|---|---|
| I — Skills + templates + minimal glue | PASS | Ships the durable artifact (the skill) via APM; no published app, no PyPI `bailiff` (vendored core). New code is only a stdlib preflight; distribution is APM's `pack`/`publish`, not bespoke tooling. |
...
| III — Faithful, agent-free reproduce | PASS (unaffected) | Packaging does not touch the reproduce path. The vendored core is the same code; an installed bailiff reproduces identically. |
...
apm.yml                  # EDIT: add a `marketplace:` block (apm marketplace init) with
                         #   marketplace.outputs.{claude,codex} + build.tagPattern. Declare the bailiff package
                         #   (local source ./packages/bailiff, version, category: Productivity [codex gate], license).

scripts/bailiff.py         # EXTEND: (1) a PEP 723 `# /// script` dependency header (copier/pyyaml/
...
src/bailiff/_preflight.py  # NEW (small): the stdlib dep-check + manager detection + suggestion builder.
                         #   Pure, testable, no third-party import (it runs BEFORE deps are guaranteed).

packages/bailiff/          # NEW (the local-source package `source: ./packages/bailiff`):
                         #   apm.yml (package metadata: type hybrid, license) + .apm/skills/bailiff/
                         #   {SKILL.md, scripts/bailiff.py, scripts/bailiff/*.py vendored} + .claude-plugin/plugin.json.
                         #   The .apm/skills/bailiff layout is GENERATED by the vendoring step, drift-checked vs src/bailiff.
                         #   (apm pack writes .claude-plugin/marketplace.json + .agents/plugins/marketplace.json at repo root.)

justfile                 # EXTEND: `just vendor` (copy src/bailiff → packages/bailiff/.apm/skills/bailiff/scripts/bailiff/)
...
skills/bailiff/SKILL.md    # EXTEND: portable Prerequisites — document the preflight/`bailiff doctor` dep check
...
README.md                # EXTEND: `## Install` — add the marketplace, install bailiff, run doctor.
...
└── loop/
    └── test_packaging.py   # NEW: apm.yml has a valid marketplace block (outputs.{claude,codex}, package
                            ...
                            #   vendored-drift check; `bailiff doctor` exit codes. (apm-dependent parts marked/guarded.)
```
...
`bailiff doctor` verb (the portability guarantee). The package layout under
`.apm/skills/bailiff/` is **generated** by a `just vendor` step from `src/bailiff/` and
