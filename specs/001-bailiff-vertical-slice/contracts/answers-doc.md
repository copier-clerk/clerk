...
`bailiff init --run-spec FILE` reads a **frozen inputs document** the agent authored
...
```yaml
# run-spec.yml
source: "https://github.com/bailiff-io/bailiff-template-example.git"   # required
...
- **`today` is injected by bailiff**, not authored here: bailiff supplies the
...
- **Trust precedence.** If the source takes actions and is untrusted, `init`
  (and `--check`) refuse with the exact `bailiff trust add <prefix>` remediation and
