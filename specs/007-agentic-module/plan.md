...
Reproduce contract (Q3): `apm.yml` re-renders **config-consistently**; `apm.lock.yaml` is
**task-generated external state**, regenerated by the pinned install task under trust
and NOT asserted config-consistent (exactly Constitution III's process-deterministic
model).
...
- `apm.yml` — rendered from the template; committed to the project; **config-reproducible**.
...
- `apm.lock.yaml` — **task side-effect only** (written by the install `_task`); NOT a
  `.jinja` render; **external state**, not config-guaranteed at reproduce (Q3).
...
- `apm.lock.yaml` is external state — reproduce config-consistency asserted for `apm.yml`
...
| III — Faithful, agent-free reproduce | PASS | `apm.yml` re-renders config-consistently from committed answers + pinned commit. The install `_task` re-runs at reproduce under trust (Constitution III: `_tasks` run at both init and reproduce). `apm.lock.yaml` is **task-generated external state** — regenerated by the pinned task, NOT asserted config-consistent (Q3). This is Constitution III's explicit "process-deterministic, not config-consistent-in-the-world" carve-out, not a violation. |
...
| VII — Hardening per-step | PASS | DoD (this spec's own): render test (correct `apm.yml` per injected set; ≥ 1 catalogue present), config-consistent reproduce test for `apm.yml`, trust-refusal test (exit 3 on untrusted source), empty-set refusal test, multi-layer ordering + `project_name` threading test against a stub base, standalone test, and `check_modules.py` compliance. No adapter → no drift test. |
...
└── loop/
    ...
    ├── test_apm_reproduce.py          # US2 — apm.yml config-consistent; lock is external
    │                                   #   state (not asserted config-consistent).
