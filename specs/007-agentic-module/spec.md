...
- Q: Reproduce contract for the network-calling APM install task (OQ-007-e) → A:
  ...
  config-reproducible rendered file. `apm.yml` re-renders config-consistently; the lock
  is regenerated by the trust-gated task and is NOT config-guaranteed. The APM tool
  ...
  III's process-deterministic (not config-consistent-in-the-world) model.
...
template stubs) — not code that runs at reproduce — replayed config-consistently at
...
config-consistently and the task re-runs (trust-gated).

**Acceptance Scenarios**:
1. **Given** a generated APM-wired project, **When** `reproduce`, **Then** `apm.yml`
   and all scaffolded files are re-rendered config-consistently (same recorded answers
   + pinned commit).
...
- **Source pinning at reproduce**: the APM `_task` install command MAY fetch
  ...
  task-generated external state, not config-guaranteed (Q3 / OQ-007-e).
...
- **`apm.lock.yaml`**: written by the APM install `_task`, NOT by the render.
  **Resolved (Q3, OQ-007-e):** the lock is treated as task-generated EXTERNAL STATE
  ...
  config-consistent. Only `apm.yml` (rendered) is config-reproducible. The APM tool
...
- **SC-002**: Reproduce re-renders `apm.yml` config-consistently from committed answers
  and re-runs the install task on a trusted source. `apm.lock.yaml` is regenerated
  by the task and is NOT asserted config-consistent (task-generated external state, Q3
...
**Resolution**: "Accept non-config-consistent lock" + "pin the APM version strictly
in the task command." `apm.lock.yaml` is task-generated external state (not
committed as reproducible state, not config-guaranteed); only `apm.yml` is
config-reproducible. This matches Constitution III. Options below retained for
context.
...
same commands), not necessarily config-consistent in the world, because tasks may touch
external state." So `apm.lock.yaml` may differ between runs if the upstream APM
resolution changes.

Options:
- **Accept non-config-consistent lock**: document this explicitly; the reproduce
...
- Constitution I (template content, not tool code — C-11: new code only for copier
  ...
  are config-consistent; task side-effects are process-deterministic — see OQ-007-e),
