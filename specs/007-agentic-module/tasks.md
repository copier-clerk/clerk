...
**Goal**: reproduce re-renders `apm.yml` config-consistently from committed answers +
pinned commit; the install `_task` re-runs under trust; `apm.lock.yaml` is treated as
external state (NOT asserted config-consistent).

**Independent Test**: generate an APM-wired project, then `reproduce` it → `apm.yml`
config-consistent; assert the lock is regenerated by the task, not diffed.
...
- [x] T013 [US2] `tests/loop/test_apm_reproduce.py` (US2 acceptance #1/#2, SC-002, Q3):
  ...
  config-consistently (same recorded `apm_packages` + pinned `_commit`). Assert the
  install `_task` re-runs under trust and that `apm.lock.yaml` is regenerated as
  external state — explicitly do NOT assert lock config-consistency. Cover N=1 (apm only)
...
**Checkpoint**: reproduce config-consistency for `apm.yml`, lock-as-external-state, and
...
- SC-002 — Reproduce re-renders `apm.yml` config-consistently; lock is external state, not
  asserted config-consistent; task pins the APM version (T013).
