# Implementation Plan: clerk Single-Template Vertical Slice

**Branch**: `001-clerk-vertical-slice` | **Date**: 2026-07-09 | **Spec**: [spec.md](./spec.md)

**Input**: Feature specification from `specs/001-clerk-vertical-slice/spec.md`
...
application (no `uvx clerk` / PyPI target this slice; the existing `pyproject.toml`
...
```text
specs/001-clerk-vertical-slice/
├── plan.md              # This file
...
├── clerk-discover        # static copier.yml + file-tree + git-ls-remote inspection → prints discovery result
...
# NOTE: no src/clerk/ package, no models.py/pydantic, no _copier_adapter.py, no schema/.
