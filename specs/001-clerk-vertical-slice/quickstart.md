...
```sh
bash scripts/try-clerk.sh          # walks every step below, pausing to inspect
bash scripts/try-clerk.sh --no-pause
KEEP=1 bash scripts/try-clerk.sh   # keep the scratch workspace to poke around
```
...
```sh
uv run clerk trust add "$TPL"    # try init first without this to see the exit-3 refusal
uv run clerk trust list
```
...
uv run clerk init --run-spec "$WORK/run-spec.yml" --check   # writes nothing
uv run clerk init --run-spec "$WORK/run-spec.yml"           # generates
...
( cd "$WORK/my-project" && uv run --project "$OLDPWD" clerk reproduce . )
