#!/usr/bin/env bash
# try-bailiff.sh — walk the whole bailiff slice-1 loop, one step at a time, pausing
...
#
# The finale demonstrates the copier-only reproduce — no bailiff, no just.
#
...
#
# Usage:
#     bash scripts/try-bailiff.sh            # pause between steps (press Enter)
#     bash scripts/try-bailiff.sh --no-pause # run straight through
#     KEEP=1 bash scripts/try-bailiff.sh     # keep the temp dir on exit

set -euo pipefail

# --- locate the bailiff repo (this script lives in <repo>/scripts) ---------------
...
# --- scratch workspace ---------------------------------------------------------
WORK="$(mktemp -d "${TMPDIR:-/tmp}/bailiff-try.XXXXXX")"
export COPIER_SETTINGS_PATH="$WORK/settings.yml"   # isolate trust store
TPL="$WORK/bailiff-template-example"
...
step "Build a local git template repo from examples/bailiff-template-example (tagged v1.0.0)"
cp -R "$REPO/examples/bailiff-template-example" "$TPL"
export GIT_AUTHOR_NAME=bailiff-try GIT_AUTHOR_EMAIL=try@bailiff.invalid
export GIT_COMMITTER_NAME=bailiff-try GIT_COMMITTER_EMAIL=try@bailiff.invalid
run git -C "$TPL" init -q
run git -C "$TPL" add -A
run git -C "$TPL" commit -qm "bailiff-template-example v1.0.0"
...
echo
echo "NOTE: no justfile or bailiff-specific file was written."
...
echo
echo "The .copier-answers.yml is the entire reproduce state."
echo "No bailiff on PATH, no justfile, no bailiff-specific file needed."
pause

step "Done"
echo "You drove the full loop: discover → trust → init → reproduce."
echo "And confirmed the copier-only fallback works without bailiff."
echo "Workspace: $WORK"
