...
The bailiff fills in the paperwork; copier makes the copies.
...
- **Not in the reproduce path.** Reproduce is agent-free: `bailiff
  ...
  with copier alone (no bailiff): `copier recopy --vcs-ref=:current: --defaults
  --overwrite` per answers file.
- **Not a template hub.** The catalog is user-owned; bailiff works against any
...
repos you want to scaffold from. bailiff depends on no first-party hub; you bring
your own templates.
...
always stays pinned. copier's 3-way merge handles local edits; bailiff supplies the
...
- **Conflicts**: if copier's 3-way merge leaves conflict markers, bailiff reports the
...
- [0001 — copier is the engine; bailiff is the conductor](docs/decisions/0001-copier-as-engine.md)
