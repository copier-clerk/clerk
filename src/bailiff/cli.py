...
def _build_parser() -> argparse.ArgumentParser:
    ...
    from bailiff import (
        __version__,  # noqa: PLC0415 — bailiff.__init__ is stdlib-only, safe pre-preflight
    )

    parser.add_argument("-V", "--version", action="version", version=f"bailiff {__version__}")
    ...
    p_catalog.add_argument(
        ...
        metavar="PATH",
        default=None,
        help="Override the catalog file path (default: BAILIFF_CATALOG_PATH or platformdirs).",
    )
...
def _cmd_doctor(_args: argparse.Namespace) -> int:
    ...
    issues = missing_or_incompatible()
    if not issues:
        print("bailiff: all dependencies present and version-compatible. Ready.")
        return 0
    print(report(issues), file=sys.stderr)
    return 4
...
def _real_dispatch(args: argparse.Namespace) -> int:  # noqa: PLR0911
    ...
    try:
        ...
        # MergeConflictError subclasses BailiffError — exit 4 so callers can distinguish
...
def main(argv: list[str] | None = None) -> int:
    """Dispatch a bailiff verb from ``argv`` (or ``sys.argv[1:]``)."""
