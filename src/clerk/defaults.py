...
Default path: ``user_config_path("clerk", appauthor=False)/defaults.yml``,
overridable via the ``CLERK_DEFAULTS_PATH`` environment variable.
...
def defaults_path() -> Path:
    ...
    When ``CLERK_DEFAULTS_PATH`` is set and the path does not exist, raises
    ...
    if env:
        ...
        return p
    return user_config_path("clerk", appauthor=False) / "defaults.yml"
