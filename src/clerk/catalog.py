...
Default path: ``user_config_path("clerk", appauthor=False)/catalog.toml``,
overridable via the ``CLERK_CATALOG_PATH`` environment variable or the
...
TOML shape::
    ...
    name = "demo"
    sources = [
        "copier-clerk/clerk-template-example",
        "acme/clerk-mod-python@v2.1.0",   # @ref: display/standardization override only
    ]
...
from clerk.errors import CatalogError, DiscoveryError

_ENV_VAR = "CLERK_CATALOG_PATH"
...
def catalog_path() -> Path:
    ...
    other clerk user-config files.
    """
    env = os.getenv(_ENV_VAR)
    if env:
        return Path(env)
    return user_config_path("clerk", appauthor=False) / "catalog.toml"
...
def save(path: Path, model: CatalogModel) -> None:
    ...
    catalog is clerk-managed config; ``tomlkit`` is the upgrade path if
