...
``settings.yml`` ``trust:``, never a blanket ``unsafe=True``). clerk's rules
(constitution V):
...
* Trust is stored in the fully-expanded ``https://`` form, because copier matches
  ...
  not match (so clerk always uses expanded URLs for both fetch and storage).
...
def settings_path() -> Path:
    ...
    Mirrors copier's own resolution so clerk reads and writes the SAME file copier
...
def list_trust() -> list[str]:
    ...
    an unordered ``set``, but clerk shows/returns it in the order the user recorded
...
def is_trusted(source: str) -> bool:
    ...
    prefix match, otherwise exact) so clerk stays on public surface and off copier's
