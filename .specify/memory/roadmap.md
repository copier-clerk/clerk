...
- **Description:** Prove the whole conduct→copier→reproduce loop for ONE trusted
  ...
  package).
- **Outcome:** With the skill, a developer inspects `clerk-mod-base`, supplies
  answers, generates a rendered + git-initialized project recording its template
  and version, and reproduces it faithfully with no agent — driven by copier's CLI
  plus small helper scripts.
- **Scope (in):**
  - The `clerk-mod-base` example template (renders identity → README/.gitignore/
...
- **Notes:** `clerk-mod-base` collapses 5 project-setup base modules; spec 009 may
  re-split. Hand-published now; recreatable in 008. One marked live smoke test; the
  rest hermetic/offline via local git fixtures.
- **Completed 2026-07-10** (branch `001-clerk-vertical-slice`): all 35 tasks done;
  ...
  `examples/clerk-mod-base/`. 35 hermetic tests + 1 network smoke (skips until
  `clerk-mod-base` is published); ruff/mypy-strict clean. Still open before
  `verified`: publish `clerk-mod-base` to run the live smoke, and a roadmap
  debrief. `clerk-mod-base` is not yet pushed to its own repo (blocks T034 live
  run and real-remote use — the catalog that points at it is spec 002).
...
- **Notes:** 001's `clerk-mod-base` collapses 5 base modules; 009 may re-split them
  into separate `clerk-mod-*` repos — kept documented.
