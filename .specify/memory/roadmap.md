...
### 001 — Single-template vertical slice  [status: planned]
...
- **Scope (in):**
  - The `clerk-mod-base` example template (renders identity → README/LICENSE/
    .gitignore/dirs; hermetic `git init` task, no commit; ships the answers-file
    `.jinja`; tagged `v1.0.0`).
  ...
  - Trust onboarding: surface copier's trust refusal (exit 4) or an equivalent
    clear error; record trust only via an explicit consent step writing the
    expanded-`https://` prefix to `settings.yml`.
